...
async fn run()
{
    ...
    let models: Vec<Model> = vec![
        ...
        Model::new("models/sphere_offset.obj", clear_glass_ggx, vec![glam::Affine3A::IDENTITY]),
    ];
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        if !state.input(&event, &window.id()) & !cam.input(&event, &window.id(), dt)
        {
            match event
            {
                ...
                {
                    data.par_iter_mut().enumerate().for_each(|(i, c): (usize, &mut glam::Vec4)| {
                        ...
                        let offset: glam::Vec2 = sobol.get_ss_sobol(state.num_samples, seed) - glam::Vec2::splat(0.5);
                        ...
                        // *c = if state.num_samples == 0 { glam::Vec4::ZERO } else { *c }
                        //     + integrator::integrate(ray, &scene, &env, &mut rng, MAX_BOUNCES);
                        let result: glam::Vec4 = integrator::integrate(ray, &scene, &env, &mut rng, MAX_BOUNCES);
                        if state.reset
                        {
                            *c = glam::Vec4::lerp(result, *c / state.num_samples as f32, 0.1);
                        }
                        else
                        {
                            *c += result;
