...
fn estimate_direct(rng: &mut TlsWyRand, bump: &Bump, r: &Ray, hit_info: &HitInfo, mat: &Material, scene: &Scene) -> glam::Vec3A
{
    ...
    // Cast material ray
    // If material ray is degenerate or points against the normal, occlusion is guaranteed and the shadow ray is skipped
    if !material_ray.direction.is_nan() && glam::Vec3A::dot(material_ray.direction, hit_info.normal) > 0.0
    {
