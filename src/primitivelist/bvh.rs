...
fn create_bounding_box(object_info: &[PrimitiveInfo]) -> AABB
{
    object_info
        .iter()
        .fold(AABB::identity(), |a: AABB, b: &PrimitiveInfo| {
            surrounding_box(&a, &b.bounding_box)
        })
}
...
impl BVHNode
{
    ...
    pub(super) fn generate_bvh(object_info: &mut [PrimitiveInfo], last_split_axis: u8) -> Self
    {
        ...
        if object_span == 1 {
            ...
            //Find longest axis
            let box_length: glam::Vec3A = bounding_box.maximum - bounding_box.minimum;
            ...
            if split_axis != last_split_axis {
                let comparator = |a: &PrimitiveInfo, b: &PrimitiveInfo| -> Ordering {
                    a.bounding_box.minimum[split_axis as usize]
                        .total_cmp(&b.bounding_box.minimum[split_axis as usize])
                };
            ...
            let (best_split, best_split_sah): (usize, f32) = (0..num_bins)
                ...
                .map(|i: usize| {
                    ...
                    let sah: f32 = TRAVERSAL_COST
                        + ((j as f32) * l_box.surface_area()
                            + ((object_span - j) as f32) * r_box.surface_area())
                            * INTERSECTION_COST
                            / bb_sa;
            ...
            if no_split_sah < best_split_sah {
                Self {
                    bounding_box,
                    node_type: NodeType::LeafMultiple {
                        primitive_indices: object_info.iter().map(|a| a.primitive_index).collect(),
                    },
                }
            } else {
                let (left_indices, right_indices): (&mut [PrimitiveInfo], &mut [PrimitiveInfo]) =
                    object_info.split_at_mut(best_split);
